...
{
       0,   127,   127,   135,   141,   148,   149,   151,   156,   161,
     166,   172,   176,   182,   186,   190,   196,   200,   207,   214,
     218,   224,   229,   231,   233,   237,   242,   245,   246,   247,
     248,   249,   250,   253,   259,   263,   269,   275,   281,   287,
     293,   300,   304,   308,   314,   318,   323,   325,   329,   334,
     339,   340,   344,   348,   352,   356,   360,   364,   368,   374,
     378,   382,   386,   392,   394,   398,   402,   408,   412,   416,
     420,   425,   430,   439,   448,   459,   460,   465,   469,   473
};
#endif
...
dlinkAppend((yyval.idList),dlinkNodeAlloc((Generic)SymIndex(currentSymtab(symStack),(yyvsp[0].name))));
}
#line 1343 "PJParser.c"
    break;

  case 10: /* IdentifierList: IdentifierList T_COMMA T_IDENTIFIER  */
#line 167 "PJParser.y"
{
dlinkAppend((yyvsp[-2].idList),dlinkNodeAlloc((Generic)SymIndex(currentSymtab(symStack),(yyvsp[0].name))));
(yyval.idList) = (yyvsp[-2].idList);
}
#line 1352 "PJParser.c"
    break;
...
#line 1584 "PJParser.c"
    break;

  case 48: /* ExitStatement: T_EXIT T_LPAREN Expr T_RPAREN  */
#line 329 "PJParser.y"
                                              {
setupFunctionReturn(instList, (yyvsp[-1].regIndex));
}
...
#line 420 "PJParser.y"
                                {
(yyval.regIndex) = generateFunctionCall(instList,(yyvsp[-2].name));
assignReturnValueToRegister(instList, (yyval.regIndex));
...
#line 431 "PJParser.y"
{			
	int symIndex = SymQueryIndex(currentSymtab(symStack),(yyvsp[0].name)); // Get symbol index from the current symbol table
	if(symIndex == -1){ // If not found, get symbol index from the last symbol table
		...
		(yyval.regIndex) = emitComputeVariableAddress(instList, symIndex, symStack, lastSymtab(symStack));
	}
	else (yyval.regIndex) = emitComputeVariableAddress(instList, symIndex, symStack, currentSymtab(symStack));
}
#line 1773 "PJParser.c"
    break;

  case 73: /* Variable: T_IDENTIFIER T_LBRACKET Expr T_RBRACKET  */
#line 440 "PJParser.y"
{
	int symIndex = SymQueryIndex(currentSymtab(symStack),(yyvsp[-3].name)); // Get symbol index from the current symbol table for array
	if(symIndex == -1){ // If not found, get symbol index from the last symbol table
	...
	else (yyval.regIndex) = emitComputeArrayAddress(instList, symIndex,(yyvsp[-1].regIndex), symStack, currentSymtab(symStack));
}
...
#line 449 "PJParser.y"
{
	int symIndex = SymQueryIndex(currentSymtab(symStack),(yyvsp[-5].name)); // Get symbol index from the current symbol table for 2D array
	if(symIndex == -1){ // If not found, get symbol index from the last symbol table
	...
	else (yyval.regIndex) = emitCompute2DArrayAddress(instList, symIndex,(yyvsp[-3].regIndex), (yyvsp[-1].regIndex), symStack, lastSymtab(symStack));
}
#line 1799 "PJParser.c"
    break;
...
#line 1832 "PJParser.c"
    break;


#line 1836 "PJParser.c"
